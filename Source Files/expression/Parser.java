...
public class Parser {
    ...
    public Node parse() throws IllegalArgumentException {
        if (cursor == 0 && spuriosCharactersCheck()) {
            throw new IllegalArgumentException("Unknown character(s) found in expression");
        }
        Token token;
    ...
    private boolean spuriosCharactersCheck() {
        return string.contains("(") ? !string.endsWith(")") : !(string.matches("^" + TokenType.VARIABLE.getRegex() + "$") || string.matches("^" + TokenType.CONSTANT.getRegex() + "$"));
    }
}
